...
module System.Metrics.Typed.Core (
...
-- data families ('AcceptorHasAgency', 'ForwarderHasAgency' and 'NobodyHasAgency')
...
-- 'AcceptorHasAgency', 'ForwarderHasAgency' and 'NobodyHasAgency'. These
...
class Protocol ps where
  ...
  --
  exclusionLemma_AcceptorAndForwarderHaveAgency
    :: forall (st :: ps).
       AcceptorHasAgency st
    -> ForwarderHasAgency st
    -> Void

  -- | Lemma that if the nobody has agency for a state, there are no
  -- cases in which the client has agency for the same state.
  --
  exclusionLemma_NobodyAndAcceptorHaveAgency
    :: forall (st :: ps).
       NobodyHasAgency st
    -> AcceptorHasAgency st
    -> Void
  ...
  -- cases in which the server has agency for the same state.
  --
  exclusionLemma_NobodyAndForwarderHaveAgency
    :: forall (st :: ps).
       NobodyHasAgency st
    -> ForwarderHasAgency st
    -> Void
...
--
data PeerRole = AsAcceptor | AsForwarder
...
data PeerHasAgency (pr :: PeerRole) (st :: ps) where
  AcceptorAgency :: !(AcceptorHasAgency st) -> PeerHasAgency AsAcceptor st
  ForwarderAgency :: !(ForwarderHasAgency st) -> PeerHasAgency AsForwarder st

instance ( forall (st' :: ps). Show (AcceptorHasAgency st')
         , forall (st' :: ps). Show (ForwarderHasAgency st')
         ) => Show (PeerHasAgency pr (st :: ps)) where
    show (AcceptorAgency stok) = "AcceptorAgency " ++ show stok
    show (ForwarderAgency stok) = "ForwarderAgency " ++ show stok
...
type family FlipAgency (pr :: PeerRole) where
  FlipAgency AsAcceptor = AsForwarder
  FlipAgency AsForwarder = AsAcceptor
...
-- For example:
--
-- > pingPongAcceptorExample :: Int -> Peer PingPong AsAcceptor StIdle m ()
-- > pingPongForwarderExample ::        Peer PingPong AsForwarder StIdle m Int
--
...
-- This takes the form of 'AcceptorAgency' or 'ForwarderAgency' applied to a
-- protocol-specific state token: either a 'AcceptorHasAgency' or a
-- 'ForwarderHasAgency' token for the protocol. The 'Done' state does not need
...
data Peer ps (pr :: PeerRole) (st :: ps) m a where
  ...
  -- Example:
  --
  -- > Yield (AcceptorAgency TokIdle) MsgPing $ ...
  --
  ...
  -- Example:
  --
  -- > Await (AcceptorAgency TokIdle) $ \msg ->
  -- > case msg of
  -- >   MsgDone -> ...
  -- >   MsgPing -> ...
  --
