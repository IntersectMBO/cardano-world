...
in
  {
    ...
    vaultPkiPath = "pki/issue/submit-api";
    consulRolePath = "consul/creds/submit-api";
  in
    with data-merge; {
      job.${id} = {
        ...
        in
          ...
          (vector.nomadTask.default {
            inherit namespace;
            endpoints = [
              # prometheus metrics for submit-api
              "http://127.0.0.1:8081"
          ...
          (
            merge node
            {
              ...
              network.port.submit-api = {to = 8070;};
              task = {
                # ----------
                # Task: Submit API
                # ----------
                submit-api = {
                  ...
                  config.image = ociNamer oci-images.cardano-submit-api;
                  user = "0:0";
                  driver = "docker";
                  kill_signal = "SIGINT";
                  kill_timeout = "30s";
