...
let
  ...
  byron-genesis = src + "/nix/cardano/config-data/mainnet/byron-genesis.json";

  #git revision (if clean) or "not-a-commit".
  gitrev = src.rev;

  # Pre-computed local project package list (generated by nix/regenerate.sh) to avoid double evaluation:
  packagesExes = import ./packages-exes.nix;
...
haskell-nix.cabalProject' ({ pkgs
  ...
  shell = {
    ...
    packages = lib.attrVals packageNames;

    #additional = lib.attrVals ["ekg-forward"];
    ...
    [
      ...
      ({ pkgs, config, ... }: {
        ...
        packages.cardano-ledger-byron.components.tests.cardano-ledger-byron-test = {
          preCheck = ''
            ...
            cp ${byron-genesis} ./mainnet-genesis.json
          '';
      ...
      })
      {
        packages = let
         ...
         in lib.genAttrs (lib.subtractLists packagesWithWarnings packageNames)
